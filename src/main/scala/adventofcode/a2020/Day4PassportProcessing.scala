...
object Day4PassportProcessing extends App {
  ...
  val lines = Input.readListString("src/main/scala/a2020/Day4PassportProcessing.in") :+ ""
  ...
  val rules: Map[String, String => Boolean] = Map(
    ...
    ("iyr", (s: String) => s.toIntOption.exists(y => y >= 2010 && s.toInt <= 2020)),
    ("eyr", (s: String) => s.toIntOption.exists(y => y >= 2020 && s.toInt <= 2030)),
    ("hgt", {
  ...
  def validate(values: Map[String, String]) = values.forall {
    case (k, v) => rules(k).apply(v)
  } && values.keys.toSet == Required

  val ans = lines.foldLeft(("", List.empty[String])) { case ((acc, normalized), newLine) =>
    if (newLine.trim.isBlank) ("", acc :: normalized)
    else (acc + " " + newLine, normalized)
  }._2.map { passportInfo =>
    passportInfo.trim.split(" ").map(l => l.substring(0, 3) -> l.substring(4).trim).toMap -- Optional
  }.count(validate)
